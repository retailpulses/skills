...
def main():
    ...
    parser.add_argument("--amazon-restock-field", default="再入荷日 (JP)")
    ...
    for row in products:
        ...
        # Rule B: Max(Qty Available, Owned Qty, Presale Qty)
        qty_available = 0
        try:
            val = row.get("Qty Available")
            if val not in (None, ""):
                qty_available = int(float(val))
        except (ValueError, TypeError):
            pass

        owned_qty = 0
        try:
            val = row.get("Owned Qty")
            if val not in (None, ""):
                owned_qty = int(float(val))
        except (ValueError, TypeError):
            pass
            
        presale_qty = 0
        try:
            val = row.get("Presale Qty")
            if val not in (None, ""):
                presale_qty = int(float(val))
        except (ValueError, TypeError):
            pass
            
        restock_date = (row.get("Restock date") or "").strip()
        
        # Logic: if Presale Qty is used, then "restock date" is mandatory
        # We take the max of all three. If presale is the max and has restock date, use it.
        # If presale is used but no restock date, we take max(qty_available, owned_qty).
        
        max_base = max(qty_available, owned_qty)
        
        if presale_qty > max_base and restock_date:
            target_qty = presale_qty
            target_restock_date = restock_date
        else:
            target_qty = max_base
            target_restock_date = ""
            
        product_map[item_code] = {
            "qty": str(target_qty),
            "restock_date": target_restock_date
        }
    ...
    for row in amazon_rows:
        ...
        current_restock = str(row.get(args.amazon_restock_field) or "").strip()
        ...
        if not item_code:
            ...
            matched_rows += 1
            target_qty = product_map[item_code]["qty"]
            target_restock = product_map[item_code]["restock_date"]

        if current_qty == target_qty and current_restock == target_restock:
            unchanged_rows += 1
            continue

        updates.append({
            "id": row["id"], 
            args.amazon_qty_field: target_qty,
            args.amazon_restock_field: target_restock
        })

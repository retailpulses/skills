...
// Helpful diagnostic when a dep is missing (e.g. agent edited package.json
// but forgot to re-run bootstrap). Surface a one-line fix instead of a stack.
function requireOrHint(name) {
  try {
    return require(name);
  } catch (e) {
    if (e && e.code === 'MODULE_NOT_FOUND') {
      console.error(
        `[html2pptx] missing dep "${name}". ` +
        `Run \`bash "{skillDir}/bootstrap.sh"\` and retry.`
      );
      process.exit(1);
    }
    throw e;
...
const { chromium } = requireOrHint('playwright');
const path = require('path');
const { fileURLToPath } = require('url');
const sharp = requireOrHint('sharp');

// Convert a possible file:// URL (as produced by the browser when resolving
// relative url('...') references against a file:// base) into a real OS path.
// On Windows the browser yields `file:///D:/x/y.png`; the previous naive
// `replace('file://', '')` left `/D:/x/y.png`, which pptxgenjs then resolved
// against the cwd as `D:\D:\x\y.png` and failed with ENOENT. fileURLToPath
// handles both POSIX and Windows correctly. Non-file paths are returned as-is.
function resolveMediaPath(p) {
  if (typeof p !== 'string') return p;
  if (p.startsWith('file:')) {
    try { return fileURLToPath(p); } catch (_) { /* fall through */ }
  }
  return p;
}
...
async function getBodyDimensions(page) {
  ...
  // Tolerance: ignore sub-pixel rounding noise from browser layout (<= 1pt).
  const OVERFLOW_TOLERANCE_PT = 1.0;
...
function validateTextBoxPosition(slideData, bodyDimensions) {
  ...
  // Tolerance: ignore sub-pixel rounding noise (~0.014" = 1pt).
  const POSITION_TOLERANCE_IN = 0.014;
  ...
  for (const el of slideData.elements) {
    ...
    if (['p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'list'].includes(el.type)) {
      ...
      if (fontSize > 12 && distanceFromBottom < minBottomMargin - POSITION_TOLERANCE_IN) {
        const getText = () => {
...
async function addBackground(slideData, targetSlide, tmpDir) {
  if (slideData.background.type === 'image' && slideData.background.path) {
    const imagePath = resolveMediaPath(slideData.background.path);
...
function addElements(slideData, targetSlide, pres) {
  for (const el of slideData.elements) {
    if (el.type === 'image') {
      const imagePath = resolveMediaPath(el.src);
...
// Module-level browser singleton: launching Chromium costs several seconds per call,
// so for multi-slide builds we reuse one browser across all slides and close it once.
let _browserPromise = null;
let _exitHookRegistered = false;

async function getBrowser(tmpDir) {
  if (_browserPromise) return _browserPromise;
  const launchOptions = { env: { TMPDIR: tmpDir } };
  if (process.platform === 'darwin') {
    launchOptions.channel = 'chrome';
  }
  _browserPromise = chromium.launch(launchOptions);
  if (!_exitHookRegistered) {
    _exitHookRegistered = true;
    // Best-effort fallback if the caller forgets to call html2pptx.close().
    process.once('beforeExit', () => { html2pptx.close().catch(() => {}); });
  }
...
async function html2pptx(htmlFile, pres, options = {}) {
  ...
  try {
    const browser = await getBrowser(tmpDir);
...
// Explicitly close the shared browser. Call this once after all html2pptx() calls
// (e.g. right before pptx.writeFile()) so the Node process exits promptly.
html2pptx.close = async function close() {

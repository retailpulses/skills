...
- Uses shared `mercari_common.graphql.MercariGraphQLClient` for SSH/direct GraphQL execution with built-in retry.
- Uses shared `mercari_common.tokens.resolve_shop_tokens` for consistent token lookup (env vars + private markdown fallback).
- Use `User-Agent: Inhouse_ERP/<VERSION>`.
- Keep tokens only in env vars, not in command history or docs.
- Shipping config mapping is fetched ONCE and reused across all shops (not per-shop).
- CSV is decoded as `utf-8-sig` first (matching the builder output), then falls back to `cp932` and `utf-8`.

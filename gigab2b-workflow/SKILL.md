...
- Do not mix this skill with unrelated OAuth-based Giga flows.
...
- Common local sources include `.env`, `wrangler.toml`, and deployment secret stores.
...
For direct shell use, run a signed GigaB2B request for smoke checks and endpoint verification:

```bash
python3 scripts/giga_request.py \
  --client-id "$GIGA_CLIENT_ID" \
  --client-secret "$GIGA_CLIENT_SECRET" \
  --endpoint "/b2b-overseas-api/v1/buyer/product/skus/v1" \
  --body '{"skus": ["ITEM_CODE_HERE"]}'
```
